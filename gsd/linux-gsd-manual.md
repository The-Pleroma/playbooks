...
## Warp + GSD on Claude Code for Linux
...
- [Part 3: Setting Up Your Linux System](#part-3-setting-up-your-linux-system)
...
This manual teaches you how to use three powerful tools together on your Linux system to work with artificial intelligence on any kind of project — whether that is building software, writing and editing a book, managing documents, doing research, or any other multi-step creative or analytical work. You do not need to know how to code. These tools handle the technical complexity for you. Your job is to be the project manager — you describe what you want, and the AI does it.

### The Two Tools

**1. Warp — Your Modern Terminal**

Warp is a modern terminal that replaces your default terminal (GNOME
Terminal, Konsole, etc.) with a faster, AI-powered experience. It
supports Bash, Zsh, and Fish. It groups command output into blocks, has
...
- A Linux distribution with glibc >= 2.31 (Ubuntu 20.04+, Fedora 32+,
  Debian 11+, Arch)
...
## Part 3: Setting Up Your Linux System

Before installing any of the tools, your system needs a few pieces of software that they all depend on: a package manager (you already have one), Node.js (a software runtime), and Git (version control). This section walks you through all of it.

### Step 1: Update Your System

Open your terminal and update packages:
...
The recommended method is using NodeSource:
...
> **CHECKPOINT**
>
> At this point you should have: updated packages, Git installed, Node.js version 18+, and npm installed. If any of these are not working, do not proceed — the remaining tools depend on them.
...
The GitHub CLI (`gh`) lets you create repos, push code, and manage everything from the terminal without opening a browser. Install it:
...
If your distro's package is outdated, install from GitHub's official repo:
```bash
...
Warp is a modern terminal application that replaces the default terminal on your Linux system (GNOME Terminal, Konsole, etc.). It was built from scratch in the Rust programming language, which makes it extremely fast. Warp adds features that default terminals do not have: AI-powered command suggestions, code-editor-style text editing, command output organized into blocks, and autocomplete for hundreds of common commands.
...
Download the .deb package from warp.dev/download, then:

```bash
sudo dpkg -i ~/Downloads/warp-terminal_*.deb
...
```bash
sudo rpm -i ~/Downloads/warp-terminal-*.rpm
```
...
Download the AppImage from warp.dev/download, then:

```bash
chmod +x ~/Downloads/Warp-*.AppImage
~/Downloads/Warp-*.AppImage
```

After installation, launch Warp from your application menu or run
`warp-terminal` from any terminal.

### Essential Warp Concepts for Beginners

**Blocks**

Every time you run a command, Warp creates a "block" that contains your command and its output. This is much cleaner than default terminals, where everything runs together in one long scroll. You can click on any block to select it, copy it, or share it.

**The Input Area**

At the bottom of the Warp window is the input area where you type commands. Unlike a default terminal, you can use your mouse to click and position your cursor anywhere in this area, just like a text editor. You can also select text, copy/paste, and use keyboard shortcuts.
...
### Useful Warp Keyboard Shortcuts (Linux)

| Shortcut | Action |
|----------|--------|
| Ctrl + T | Open a new tab |
| Ctrl + D | Split the pane vertically |
| Ctrl + Shift + D | Split the pane horizontally |
...
>
> You can technically do everything in GNOME Terminal, Konsole, Alacritty, or any other terminal. Warp simply makes it easier, especially for beginners. The AI suggestions, the block-based output, and the text-editor-style input area remove a lot of friction. If you are comfortable with your current terminal, you can use that instead — all the commands in this manual will work the same way.
...
1. Open Warp (or any terminal) and type: `claude`
2. Claude Code will try to open a browser window. If it does not open automatically, it will display a URL. Copy and paste it into your browser.
3. Log in to your Claude account (or Anthropic console account) in the browser.
4. Authorize Claude Code when prompted.
5. Return to your terminal. Claude Code should now be authenticated and ready to use.
...
> **UNDERSTAND THE RISK**
>
> The `--dangerously-skip-permissions` flag lets Claude Code read, write, and execute commands without asking you first. Only use this in project folders where you are comfortable giving the AI full access. Never use this in sensitive directories like your home folder or system directories like `/` or `/etc`.
...
1. Save the .skill file to your Linux system
...
Global installation places GSD in your home Claude folder (`~/.claude/`), making it available in every project on your system.
...
Create your project folder using your file manager or the terminal. Use a lowercase name like `my-portfolio`. Then navigate to it in Warp:

```bash
mkdir ~/Developer/my-portfolio
...
**Scenario A: Your project files are on your Linux system already**

If you have a folder with your project files (code, documents, assets, etc.), you are ready. Just note the folder path. For example: `~/Documents/my-app` or `~/Developer/my-project`.

**Scenario B: Your project files were created in Claude's web chat**

If you built your project in Claude's web chat or desktop app, the files Claude created need to be saved to your system. Claude's chat interface lets you download individual files or artifacts. You need to:
...
3. Move the downloaded files into your project folder. You can drag and drop them from your file manager into the folder, or use Warp:
...
>
> If you are unsure how to organize your files, you can ask Claude in the web chat: "List all the files you created for this project and show me the folder structure." Then recreate that structure on your system.

**Scenario C: Your project is in a GitHub or remote repository**

If your project is hosted on GitHub, GitLab, or another Git hosting service, clone it:
...
This means Node.js is not installed or not in your PATH. Reinstall Node.js using the instructions in Part 3.
...
**Q: Can I use a different terminal instead of Warp?**
A: Yes. GNOME Terminal, Konsole, Alacritty, Kitty, or any terminal works. Warp just makes the experience nicer.
...
1. **Disable Adaptive Thinking** — add this environment variable to your shell profile (`~/.bashrc` or `~/.zshrc`):
```bash
echo 'export claude_code_disable_adaptive_thinking=1' >> ~/.bashrc
source ~/.bashrc
```
...
A: Yes. Everything in this manual works on Ubuntu, Fedora, Debian, Arch, and other distributions with glibc >= 2.31. The tools are not hardware-intensive — the AI processing happens on Anthropic's servers, not your machine.

**Q: Can I use this on macOS or Windows?**
A: Warp and Claude Code both support macOS, Windows, and Linux. GSD works on all three platforms. Separate manuals are available for each platform.
...
- Obsidian installed (https://obsidian.md — free, available as AppImage, .deb, .snap, or Flatpak)
...
"mcpServers": {
    "obsidian": {
      "command": "/home/YOUR_USERNAME/Documents/Obsidian Vault/.obsidian/plugins/mcp-tools/bin/mcp-server",
      "env": {
...
**IMPORTANT:** Replace `YOUR_USERNAME` with your actual Linux username, adjust the vault path if yours is different, and paste your actual API key.
...
| Claude Desktop config | `~/.config/Claude/claude_desktop_config.json` (auto-configured) |
...
Open Warp (or any terminal) and run:

```bash
obsidian version
```

You should see something like `1.12.7 (installer 1.12.7)`. If you get "command not found," the binary may not be in your PATH. Try adding this to your `~/.bashrc` (or `~/.zshrc` if you use Zsh):

```bash
export PATH="$PATH:/usr/bin"
```

If you installed Obsidian as an AppImage or Flatpak, the CLI path may differ. Check where your Obsidian binary is located:

```bash
which obsidian || find /usr -name "obsidian" 2>/dev/null
```

Then add the correct path to your shell config and run `source ~/.bashrc`.
...
| What | Location |
|------|----------|
| CLI binary | Depends on install method (AppImage, .deb, snap, etc.) — check with `which obsidian` |
...
| Step | Command |
|------|---------|
| Update system (Ubuntu/Debian) | `sudo apt update && sudo apt upgrade -y` |
| Update system (Fedora) | `sudo dnf update -y` |
| Update system (Arch) | `sudo pacman -Syu` |
| Install Git (Ubuntu/Debian) | `sudo apt install git -y` |
| Install Node.js (Ubuntu/Debian) | `curl -fsSL https://deb.nodesource.com/setup_20.x \| sudo -E bash - && sudo apt install nodejs -y` |
| Install Claude Code | `curl -fsSL https://claude.ai/install.sh \| bash` |
| Install GSD globally | `npx get-shit-done-cc --claude --global` |
| Download Warp | warp.dev/download |

### GSD Commands (run inside Claude Code)
...
| Step | Command | Where |
|------|---------|-------|
| 1. Create folder | `mkdir ~/Developer/project` | Terminal |
| 2. Navigate | `cd ~/Developer/project` | Terminal |
| 3. Init Git | `git init` | Terminal |
| 4. Launch Claude | `claude --dangerously-skip-permissions` | Warp |
...
| 11. Repeat 9-10 | `/gsd:plan-phase N` + `/gsd:execute-plan` | Claude Code |
...
> **REMEMBER**
>
> Warp is where you type. Claude Code is the AI that builds. GSD keeps the AI focused. You are the project manager.
